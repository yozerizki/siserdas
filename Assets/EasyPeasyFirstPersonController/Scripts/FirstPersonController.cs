namespace EasyPeasyFirstPersonController
{
    ...
    public partial class FirstPersonController : MonoBehaviour
    {
        ...
        private IInputManager ResolveInputManager()
        {
            var allBehaviours = GetComponents<MonoBehaviour>();
            var managers = new List<IInputManager>();

            foreach (var behaviour in allBehaviours)
            {
                if (behaviour is IInputManager manager)
                    managers.Add(manager);
            }

            if (managers.Count == 0)
            {
                Debug.LogError("FirstPersonController: No component implementing IInputManager found on this GameObject.");
                return null;
            }

            foreach (var manager in managers)
            {
                var behaviour = manager as MonoBehaviour;
                if (behaviour != null && behaviour.enabled)
                    return manager;
            }

            Debug.LogWarning("FirstPersonController: IInputManager found but all are disabled. Using first one.");
            return managers[0];
        }

        private bool CheckGroundedSafe()
        {
            ...
            // Ground probe from CharacterController feet fixes spawn-frame false negatives
            // before the first Move() call updates CharacterController.isGrounded.
            bool feetProbeGrounded = false;
            if (characterController != null)
            {
                ...
                float halfHeight = Mathf.Max(characterController.height * 0.5f, probeRadius);
                float probeDistance = (halfHeight - probeRadius) + characterController.skinWidth + 0.08f;

                feetProbeGrounded = Physics.SphereCast(
                    worldCenter,
                    probeRadius,
                    Vector3.down,
                    out _,
            ...
            return controllerGrounded || sphereGrounded || feetProbeGrounded;
        }
